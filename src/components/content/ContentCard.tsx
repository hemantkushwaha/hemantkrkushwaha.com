...
export function ContentCard({ item, id }: ContentCardProps) {
  ...
  return (
    <Link
      ...
      id={id || `content-card-link-${item.slug}`}
      className="block h-full focus-visible:outline-2 focus-visible:outline-stone-900 focus-visible:outline-offset-2 rounded-xl"
    >
      <Card
        id={id ? `${id}-card` : `content-card-${item.slug}`}
        interactive={true}
        className="flex flex-col justify-between h-full group transition-all duration-200 hover:border-stone-400 hover:shadow-md cursor-pointer"
      >
